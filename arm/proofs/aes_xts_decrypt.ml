...
(* The ending pc will be an if-then-else depending on which branch it goes to *)
let AES_XTS_DECRYPT_CORRECT = prove(
  `!ct_ptr pt_ptr ct pt_init key1_ptr key2_ptr iv_ptr iv len
     ...
     *)
    ASM_CASES_TAC `len < 80` THENL [CHEAT_TAC; ALL_TAC] THEN
    ...
    *)
    ...
    SUBGOAL_THEN `~(~(ival (word_sub (len:int64) (word 0x10)) < &0x0 <=>
      ~(ival len - &0x10 = ival (word_sub len (word 0x10)))))` ASSUME_TAC THENL
    [CHEAT_TAC;ALL_TAC] THEN
