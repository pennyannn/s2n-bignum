...
let mlkem_keccak_f1600_mc_5 = new_definition `mlkem_keccak_f1600_mc_5 = [
    word 0x53; 
    word 0x55; 
    word 0x41; word 0x54; 
    word 0x41; word 0x55; 
...
let EXEC = X86_MK_EXEC_RULE mlkem_keccak_f1600_mc_5;;


let MLKEM_KECCAK_F1600_SPEC = prove(
  `forall pc:num stackpointer:int64 rdi. 
  P[stackpointer] /\ 
  nonoverlapping_modulo (2 EXP 64) (pc, 0x32) (val (word_sub stackpointer (word 248):int64),248) /\
  nonoverlapping_modulo (2 EXP 64) (pc, 0x32) (val (word_add (rdi) (word 0):int64),200)
    ==> ensures x86
  // Precondition
  (\s. bytes_loaded s (word pc) mlkem_keccak_f1600_mc_5 /\
       read RIP s = word pc /\
       read RSP s = stackpointer /\
       read RDI s = rdi)
  // Postcondition
  (\s. read RIP s = word (pc+0x32))
  (MAYCHANGE [RIP;RSP;RBX;RBP;R12;R13;R14;R15;RDI] ,, MAYCHANGE SOME_FLAGS ,, 
